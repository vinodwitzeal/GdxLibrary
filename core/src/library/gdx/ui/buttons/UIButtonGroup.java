 ...
 * <p>
 * The {@link #canCheck(UIButton, boolean)} method can be overridden to control if a button check or uncheck is allowed.
 * @author Nathan Sweet */
public class UIButtonGroup<T extends UIButton> {
    ...
    public UIButtonGroup (T... buttons) {
        minCheckCount = 0;
        add(buttons);
        minCheckCount = 1;
    }
    ...
    /** Sets the first {@link UITextButton} with the specified text to checked. */
    public void setChecked (String text) {
        if (text == null) throw new IllegalArgumentException("text cannot be null.");
        for (int i = 0, n = buttons.size; i < n; i++) {
            T button = buttons.get(i);
            if (button instanceof UITextButton && text.contentEquals(((UITextButton)button).getText())) {
    ...
    /** Sets all buttons' {@link UIButton#isChecked()} to false, regardless of {@link #setMinCheckCount(int)}. */
